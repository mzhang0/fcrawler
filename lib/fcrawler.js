...
FocusedCrawler.prototype.crawl = function(inputObj) {
  ...
  this._classifier = inputObj.classifier;
  this._classifierLabels = inputObj.classifierLabels;
...
FocusedCrawler.prototype._filterLinks = function(links, body) {
  ...
  var priority;
  var $ = cheerio.load(body);
  var docTerms = natural.PorterStemmer
    .tokenizeAndStem($('body').text());

  if (this._classifier != undefined) 
    priority = this._computeDocProb(docTerms);
  else priority = this._computeDocSim(docTerms);
  
  _.forEach(links, function(link) {
    var linkObj = url.parse(link);
    if (!linkObj["query"] && that._isAllowedDomain(linkObj)) {
      linkValues[link] = priority;
    }
  });
...
FocusedCrawler.prototype._computeDocProb = function(docTerms) {
  ...
  var classifications = this._classifier.getClassifications(docTerms);
  var classObj = _.find(classifications, function(classObj) {
    return classObj.label === that._classifierLabels.relevant;
  });
  return 1 - classObj.value;
};

FocusedCrawler.prototype._computeDocSim = function(docTerms) {
  var that = this;
  var dTermFreqs = _.countBy(docTerms);
  var numerSum = 0;
  var denomDocSum = 0;
  ...
  if (this._shouldFetchLink(link)) {
    if (this.config.robotsCompliance) {
      ...
      if (this._robotstxtCache.has(rLink)) {
        var rules = this._robotstxtCache.get(rLink);
        if (rules != null ||
          robotto.check(this.config.userAgent, link, rules))
        ...
        this._crawlerRequest.get(rLink, function(error, response, body) {
          if (error) {
            ...
            var rules = robotto.parse(body)
            that._robotstxtCache.set(rLink, rules);
            if (robotto.check(that.config.userAgent, link, rules))
              that._fetchLink(link);
            else
